...
# -*- coding: utf-8 -*-
"""
Grouped district-level Prophet benchmark for monthly dengue forecasting.
...
- For monthly data, Prophet docs recommend making only monthly forecasts (`freq='MS'`) and,
  when appropriate, representing yearly seasonality with month indicator regressors rather than
  built-in yearly seasonality to avoid within-month identifiability problems. Official docs also
  support component plots, regressor coefficient extraction, and forecast intervals. citeturn813191view0turn333177view0turn266286view1
"""
...
def write_run_summary(cfg: Config, windows, out_dir: Path):
    ...
    lines += [
        ...
        "If tuning is enabled, validation-window rolling forecasts are used to choose changepoint_prior_scale, seasonality_prior_scale, and seasonality_mode per district.",
    ]
...
def parse_args() -> Config:
    ...
    p.add_argument("--output_dir", type=str, default="Prophet/outputs")
...
def main():
    ...
    for district, g in df.groupby(cfg.district_col):
        selected_params, tuning_df = tune_prophet_for_district(g.copy(), cfg, date_to_split, y_lookup)
        selected_params_by_district[str(district)] = selected_params or {}
        if tuning_df is not None and len(tuning_df):
            tuning_frames.append(tuning_df)
        pred_df, audit_df = rolling_forecasts_for_district(g.copy(), cfg, date_to_split, y_lookup, selected_params=selected_params)
    ...
    tuning_all = pd.concat(tuning_frames, ignore_index=True) if tuning_frames else pd.DataFrame()

    save_audit_tables(audit_all, out_dir)
    save_tuning_tables(tuning_all, out_dir)
    ...
    save_final_train_diagnostics(df, cfg, out_dir, selected_params_by_district)
