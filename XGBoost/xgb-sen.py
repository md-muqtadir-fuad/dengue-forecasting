...
# -*- coding: utf-8 -*-
"""
Direct multi-horizon XGBoost for the dengue district-panel pipeline.
...
def build_horizon_sample(
    ...
    # target-month seasonality for direct multi-step forecasting
...
def param_candidates(cfg: Config):
    grid = {
        "n_estimators": [600, 900, 1200, 1800, 2500],
...
def make_model(cfg: Config, params: Dict | None = None) -> xgb.XGBRegressor:
...
def fit_one_horizon(sample: pd.DataFrame, features: List[str], cfg: Config, h: int, out_dir: Path, mase_denom: float) -> Dict:
    ...
    if cfg.use_tuning:
        ...
        for params in param_candidates(cfg):
            ...
            model.fit(X_train, y_train_log, sample_weight=w_train, eval_set=[(X_val, y_val_log)], verbose=False)
    ...
    model.fit(X_train, y_train_log, sample_weight=w_train, eval_set=[(X_val, y_val_log)], verbose=False)
    model.get_booster().save_model(str(out_dir / f"xgb_model_h{h}.json"))
    ...
    # percentage skill scores for easier manuscript use
    ...
    gain = pd.Series(model.get_booster().get_score(importance_type="gain"), dtype=float).reindex(horizon_features).fillna(0.0)
    X_shap = X_test.copy()
    if len(X_shap) > 300:
    ...
    shap_imp = pd.DataFrame(shap_values, columns=horizon_features).abs().mean().sort_values(ascending=False)
...
def save_prediction_tables(results, out_dir: Path):
    ...
    pred_train.to_csv(out_dir / "xgb_train_predictions_long.csv", index=False)
    pred_val.to_csv(out_dir / "xgb_val_predictions_long.csv", index=False)
    pred_test.to_csv(out_dir / "xgb_test_predictions_long.csv", index=False)
    pred_test.to_csv(out_dir / "xgb_test_residuals_long.csv", index=False)
...
def save_split_manifest(windows, results, out_dir: Path):
    ...
    pd.DataFrame(rows).to_csv(out_dir / "xgb_split_manifest.csv", index=False)
...
def save_metrics(results, preds, out_dir: Path, mase_denom: float):
    ...
    for split, df_ in preds.items():
        ...
        m.update({"Model": "XGBoost", "Split": split})
        rows.append(m)
    ...
    pd.DataFrame(rows)[["Model", "Split", "n", "RMSE", "MAE", "R2", "CVRMSE", "NRMSE", "sMAPE", "MBE", "MedAE", "MASE"]].to_csv(out_dir / "xgb_summary.csv", index=False)
    ...
    for r in results:
        ...
        per_h.append(row)
    pd.DataFrame(per_h).sort_values("horizon").to_csv(out_dir / "xgb_per_horizon.csv", index=False)
    ...
    pd.DataFrame(regime_rows).sort_values(["horizon", "regime"]).to_csv(out_dir / "xgb_regimewise_metrics.csv", index=False)
    ...
    pd.DataFrame(district_rows).sort_values("District").to_csv(out_dir / "xgb_metrics_by_district.csv", index=False)
    ...
    for (district, h), g in preds["test"].groupby(["District", "horizon"]):
        ...
        dh_rows.append(m)
    pd.DataFrame(dh_rows).sort_values(["District", "horizon"]).to_csv(out_dir / "xgb_metrics_by_district_horizon.csv", index=False)
    ...
    for r in results:
        ...
        cls_rows.append(cls)
    pd.DataFrame(cls_rows).sort_values("horizon").to_csv(out_dir / "xgb_outbreak_classification_metrics.csv", index=False)


def save_importance(results, out_dir: Path):
    features = results[0]["feature_cols"]
    gain_df = pd.DataFrame(index=features)
    shap_df = pd.DataFrame(index=features)
    for r in results:
        gain_df[f"h{r['horizon']}"] = r["gain_importance"].reindex(features).fillna(0.0).values
    ...
    shap_df.sort_values("mean_abs_shap", ascending=False).to_csv(out_dir / "shap_aggregate_importance.csv")
...
def save_row_audit(results, out_dir: Path):
    ...
    if rows:
        pd.DataFrame(rows).sort_values("horizon").to_csv(out_dir / "xgb_row_audit.csv", index=False)
...
def make_figures(results, preds, out_dir: Path):
    ...
    plt.title("XGBoost Test: True vs Predicted")
    ...
    per_h = pd.read_csv(out_dir / "xgb_per_horizon.csv")
    ...
    # 6. SHAP plots for edge horizons
    ...
    # 7. Aggregate importance plots
    gain = pd.read_csv(out_dir / "xgb_feature_importance_gain.csv", index_col=0)["mean_gain"].sort_values(ascending=False).head(20).iloc[::-1]
    gain.index = gain.index.map(pretty_column_name)
    plt.figure(figsize=(10, 8))
    gain.plot(kind="barh")
    plt.title("Top 20 Mean Gain Importance (All Horizons)")
    plt.xlabel("Mean Gain")
    plt.ylabel("Feature")
    plt.tight_layout()
    plt.savefig(out_dir / "feature_importance_gain_bar.pdf", dpi=300)
    plt.close()

    sagg = pd.read_csv(out_dir / "shap_aggregate_importance.csv", index_col=0)["mean_abs_shap"].sort_values(ascending=False).head(20).iloc[::-1]
...
def load_fixed_config(path: str) -> Dict:
...
def build_ablation_features(base_features: List[str], ablation: str) -> Tuple[List[str], bool, List[str]]:
    ...
    if ablation == "no_climate":
        removed = [f for f in feats if f in CLIMATE_FEATURES]
        feats = [f for f in feats if f not in CLIMATE_FEATURES]
        include_target_month = True
    elif ablation == "no_serotype":
        removed = [f for f in feats if f in SEROTYPE_FEATURES]
        feats = [f for f in feats if f not in SEROTYPE_FEATURES]
        include_target_month = True
    elif ablation == "no_population_density":
        removed = [f for f in feats if f in POP_FEATURES]
        feats = [f for f in feats if f not in POP_FEATURES]
        include_target_month = True
    elif ablation == "no_temporal":
        removed = ["TargetMonth_sin", "TargetMonth_cos"]
        include_target_month = False
    else:
        include_target_month = True

    if not feats and not include_target_month:
        raise ValueError("Ablation removed all features. This is not allowed.")

    return feats, include_target_month, removed
...
def fit_one_horizon(
    ...
    include_target_month: bool = True,
) -> Dict:
    train_df = sample[sample["target_split"] == "train"].copy()
    val_df = sample[sample["target_split"] == "val"].copy()
    test_df = sample[sample["target_split"] == "test"].copy()
    ...
    if include_target_month:
        for extra in ["TargetMonth_sin", "TargetMonth_cos"]:
            if extra in sample.columns and extra not in horizon_features:
                horizon_features.append(extra)
    ...
    model = make_model(cfg, fixed_params)
    model.fit(X_train, y_train_log, sample_weight=w_train, eval_set=[(X_val, y_val_log)], verbose=False)
    model.get_booster().save_model(str(out_dir / f"xgb_model_h{h}.json"))
    ...
    return {
        ...
        "row_audit": {
            **sample.attrs.get("row_audit", {}),
            ...
            "val_rows": len(val_df),
            "test_rows": len(test_df),
            "feature_count": len(horizon_features),
            "include_target_month": include_target_month,
        },
        "gain_importance": gain,
        "shap_values": shap_values,
        "shap_test_X": X_shap,
...
def write_sensitivity_run_summary(cfg, windows, features, out_dir: Path, ablation: str, removed: List[str], include_target_month: bool, seed: int):
    summary_features = list(features)
    if include_target_month:
        summary_features += ["TargetMonth_sin", "TargetMonth_cos"]
    summary_features = list(dict.fromkeys(summary_features))

    lines = [
        "XGBoost sensitivity run summary",
        "=" * 80,
        f"Input file: {cfg.input_path}",
        f"Ablation: {ablation}",
        f"Seed: {seed}",
    ...
    lines += [
        ...
        *summary_features,
        ...
        "Note",
        "-" * 80,
        "TargetMonth_sin and TargetMonth_cos are horizon-specific seasonal features derived from TargetDate.",
        f"Target-month branch included: {include_target_month}",
    ]
    (out_dir / "run_summary.txt").write_text("\n".join(map(str, lines)), encoding="utf-8")


def summarize_experiment(preds: Dict[str, pd.DataFrame], mase_denom: float, ablation: str, seed: int, experiment: str, feature_count: int, include_target_month: bool) -> Dict:
    test = preds["test"]
...
def aggregate_sensitivity_outputs(root_dir: Path, experiment_rows: List[Dict], per_h_rows: List[Dict], feature_rows: List[Dict]) -> None:
    ...
    if not exp_df.empty:
        exp_df.to_csv(root_dir / "xgb_sensitivity_experiments.csv", index=False)
        summary = (
        ...
        summary.to_csv(root_dir / "xgb_sensitivity_summary_by_ablation.csv", index=False)

    if not perh_df.empty:
        perh_df.to_csv(root_dir / "xgb_sensitivity_per_horizon_long.csv", index=False)
        perh_summary = (
        ...
        perh_summary.to_csv(root_dir / "xgb_sensitivity_per_horizon_summary.csv", index=False)

    if not feat_df.empty:
        feat_df.to_csv(root_dir / "xgb_sensitivity_feature_manifest.csv", index=False)
...
def run_experiment(
    ...
    ablated_features, include_target_month, removed = build_ablation_features(base_features, ablation)
    write_sensitivity_run_summary(cfg_exp, windows, ablated_features, exp_dir, ablation, removed, include_target_month, seed)
    ...
    for h in range(1, cfg_exp.horizon + 1):
        ...
        fixed_params = dict(h_cfg.get("params", {}))
        fixed_params["random_state"] = seed
        fixed_threshold = h_cfg.get("outbreak_threshold", None)

        sample = build_horizon_sample(base_df, ablated_features, cfg_exp, h, date_to_split)
        res = fit_one_horizon(
            sample=sample,
    ...
    save_json(fixed_out, exp_dir / "xgb_sensitivity_used_params.json")
    ...
    exp_summary = summarize_experiment(
        ...
        seed=seed,
        experiment=exp_name,
        feature_count=len(ablated_features) + (2 if include_target_month else 0),
        include_target_month=include_target_month,
    )
    ...
    for r in results:
        ...
        row.update(r["metrics_test"])
        row.update(r["classification_test"])
        row["feature_count"] = len(r["feature_cols"])
        row["include_target_month"] = include_target_month
    ...
    feature_row = {
        ...
        "features_used": "|".join(list(ablated_features) + (["TargetMonth_sin", "TargetMonth_cos"] if include_target_month else [])),
    }
    return exp_summary, per_h_rows, feature_row


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="XGBoost sensitivity analysis for the dengue district-panel pipeline.")
    p.add_argument("--input", type=str, default="./data/raw/prime_dataset_model_input_with_purge.csv")
    p.add_argument("--output_dir", type=str, default="XGBoost/outputs/sensitivity")
...
def main():
    ...
    fixed_cfg = load_fixed_config(args.fixed_config_json)
