...
class Config:
    ...
    ablation_names: str = "full,no_climate,no_serotype,no_temporal,no_population_density"
    seed_list: str = "42"
...
def load_fixed_tft_config(path: str | None, cfg: Config) -> Dict[str, Any]:
    if not path:
        base = dict(cfg.base_model_config)
        base["batch_size"] = cfg.batch_size
        base["gradient_clip_val"] = cfg.gradient_clip_val
        return base
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    required = {"hidden_size", "attention_head_size", "dropout", "hidden_continuous_size", "learning_rate"}
    missing = sorted(required - set(data.keys()))
    if missing:
        raise ValueError(f"Fixed TFT config is missing required fields: {missing}")
    if "batch_size" not in data:
        data["batch_size"] = cfg.batch_size
    if "gradient_clip_val" not in data:
        data["gradient_clip_val"] = cfg.gradient_clip_val
    return data
...
def parse_ablation_names(text_: str) -> List[str]:
    names = []
    for x in str(text_).split(','):
        x = x.strip()
        if x:
            names.append(x)
    names = names or ["full"]
    allowed = {"full", "no_climate", "no_serotype", "no_temporal", "no_population_density"}
    bad = sorted(set(names) - allowed)
    if bad:
        raise ValueError(f"Unknown ablations: {bad}. Allowed: {sorted(allowed)}")
    return names
...
POP_FEATURES = {"PopulationDensity"}
TEMPORAL_KNOWN_FEATURES = {"TargetMonth_sin", "TargetMonth_cos"}


def apply_ablation_to_schema(schema: Dict[str, List[str]], ablation: str) -> Tuple[Dict[str, List[str]], List[str]]:
    new_schema = {k: list(v) for k, v in schema.items()}
    removed: List[str] = []

    if ablation == "no_climate":
        removed = [c for c in new_schema["time_varying_unknown_reals"] if c in CLIMATE_FEATURES]
        new_schema["time_varying_unknown_reals"] = [c for c in new_schema["time_varying_unknown_reals"] if c not in CLIMATE_FEATURES]
    elif ablation == "no_serotype":
        removed = [c for c in new_schema["time_varying_unknown_reals"] if c in SEROTYPE_FEATURES]
        new_schema["time_varying_unknown_reals"] = [c for c in new_schema["time_varying_unknown_reals"] if c not in SEROTYPE_FEATURES]
    elif ablation == "no_population_density":
        removed = [c for c in new_schema["static_reals"] if c in POP_FEATURES]
        new_schema["static_reals"] = [c for c in new_schema["static_reals"] if c not in POP_FEATURES]
    elif ablation == "no_temporal":
        removed = [c for c in new_schema["time_varying_known_reals"] if c in TEMPORAL_KNOWN_FEATURES]
        new_schema["time_varying_known_reals"] = [c for c in new_schema["time_varying_known_reals"] if c not in TEMPORAL_KNOWN_FEATURES]
    elif ablation == "full":
        removed = []
    else:
        raise ValueError(f"Unknown ablation: {ablation}")

    if cfg.time_idx_col if False else False:  # no-op placeholder to keep formatter calm
        pass

    return new_schema, removed


def write_sensitivity_run_summary(cfg: Config, windows: Dict[str, List[str]], schema: Dict[str, List[str]], out_dir: Path, ablation: str, removed: List[str], fixed_cfg: Dict[str, Any], seed: int):
    features = schema["static_reals"] + schema["time_varying_known_reals"] + schema["time_varying_unknown_reals"]
    lines = [
        "Temporal Fusion Transformer sensitivity run summary",
        "=" * 80,
        ...
        f"Fixed config json: {cfg.fixed_config_json}",
        "",
    ...
    lines += [
        "",
        "Removed features / branches",
        "-" * 80,
        *(removed if removed else ["<none>"]),
        "",
        "Fixed model config",
        "-" * 80,
        json.dumps(fixed_cfg, indent=2),
        "",
        "Variable schema",
        "-" * 80,
        ...
        *features,
        ...
        "For TFT no_temporal removes the target-month known-future branch, because source Year / Month_sin / Month_cos are already excluded in the main pipeline.",
    ]
...
def run_one_experiment(
    ...
    exp_cfg = Config(**{**base_cfg.__dict__})
    exp_cfg.output_dir = str(exp_dir)
    exp_cfg.random_state = seed
    exp_cfg.use_tuning = False

    seed_everything(exp_cfg.random_state, workers=True)

    fixed_cfg = load_fixed_tft_config(exp_cfg.fixed_config_json, exp_cfg)
    exp_cfg.batch_size = int(fixed_cfg.get("batch_size", exp_cfg.batch_size))
    exp_cfg.gradient_clip_val = float(fixed_cfg.get("gradient_clip_val", exp_cfg.gradient_clip_val))

    df_tft, time_idx_to_date, actual_log_lookup, actual_count_lookup, schema = prepare_tft_frame(df, exp_cfg)
    schema, removed = apply_ablation_to_schema(schema, ablation)
    write_sensitivity_run_summary(exp_cfg, windows, schema, exp_dir, ablation, removed, fixed_cfg, seed)

    training, validation, test, train_eval = build_datasets(df_tft, exp_cfg, schema, exp_dir)
    train_loader, val_loader, test_loader, train_eval_loader = make_dataloaders(training, validation, test, train_eval, exp_cfg)

    final_model, trainer, best_ckpt, metrics_path = fit_tft_candidate(
        training, train_loader, val_loader, exp_cfg, fixed_cfg, exp_dir / "final_fit", f"tft_{ablation}_seed{seed}"
    )
    canonical_ckpt = exp_dir / "tft_model.ckpt"
    ...
    plot_training_history(metrics_df, exp_dir)
    save_json(fixed_cfg, exp_dir / "tft_best_config.json")
    save_variable_importance(final_model, exp_dir)

    train_q, train_index = predict_quantiles(final_model, train_eval_loader, exp_cfg)
    val_q, val_index = predict_quantiles(final_model, val_loader, exp_cfg)
    test_q, test_index = predict_quantiles(final_model, test_loader, exp_cfg)

    pred_train = prediction_to_long(train_q, train_index, "train", exp_cfg, time_idx_to_date, actual_log_lookup, actual_count_lookup, build_date_split_map(df, exp_cfg))
    pred_val = prediction_to_long(val_q, val_index, "val", exp_cfg, time_idx_to_date, actual_log_lookup, actual_count_lookup, build_date_split_map(df, exp_cfg))
    pred_test = prediction_to_long(test_q, test_index, "test", exp_cfg, time_idx_to_date, actual_log_lookup, actual_count_lookup, build_date_split_map(df, exp_cfg))

    add_outbreak_thresholds(pred_train, pred_val, pred_test)
    preds = save_prediction_tables(pred_train, pred_val, pred_test, exp_dir)
    save_split_manifest(pred_train, pred_val, pred_test, windows, exp_dir)
    save_row_audit(pred_train, pred_val, pred_test, exp_dir)
    mase_denom = compute_panel_mase_denom(df_tft, exp_cfg)
    save_metrics(preds, exp_dir, mase_denom, exp_cfg.quantiles)

    raw_test = predict_raw(final_model, test_loader, exp_cfg)
    save_plot_predictions(final_model, raw_test, exp_dir, n_examples=6)
    save_interpretation(final_model, raw_test, exp_dir)
    make_figures(preds, exp_dir)

    exp_summary = compute_metrics(preds["test"]["y_true_count"], preds["test"]["y_pred_count_p50"], mase_denom)
    exp_summary.update({
        "experiment": exp_name,
        "ablation": ablation,
        "seed": seed,
        "feature_count": len(schema["static_reals"] + schema["time_varying_known_reals"] + schema["time_varying_unknown_reals"]),
        "removed_features": "|".join(removed),
    })

    per_h_df = pd.read_csv(exp_dir / "tft_per_horizon.csv")
    per_h_df.insert(0, "seed", seed)
    per_h_df.insert(0, "ablation", ablation)
    per_h_df.insert(0, "experiment", exp_name)

    archive = shutil.make_archive(str(exp_dir), "zip", root_dir=exp_dir)
    exp_summary["zip_archive"] = archive
    return exp_summary, per_h_df


def save_sensitivity_root_tables(root_out_dir: Path, experiment_rows: List[Dict[str, Any]], per_horizon_parts: List[pd.DataFrame]) -> None:
    exp_df = pd.DataFrame(experiment_rows).sort_values(["ablation", "seed"]).reset_index(drop=True)
    exp_df.to_csv(root_out_dir / "tft_sensitivity_experiments.csv", index=False)

    if per_horizon_parts:
        per_h_df = pd.concat(per_horizon_parts, ignore_index=True)
        per_h_df.to_csv(root_out_dir / "tft_sensitivity_per_horizon_long.csv", index=False)
        summary = (
            ...
            .agg(
                ...
                Coverage80_std=("Coverage80", "std"),
                runs=("experiment", "nunique"),
            )
            .sort_values(["ablation", "horizon"])
        )
        summary.to_csv(root_out_dir / "tft_sensitivity_per_horizon_summary.csv", index=False)

    agg_rows = []
    for ablation, g in exp_df.groupby("ablation"):
        agg_rows.append({
            "ablation": ablation,
            "runs": int(len(g)),
            "test_RMSE_mean": g["RMSE"].mean(),
            "test_RMSE_std": g["RMSE"].std(),
            "test_MAE_mean": g["MAE"].mean(),
            "test_MAE_std": g["MAE"].std(),
            "test_R2_mean": g["R2"].mean(),
            "test_R2_std": g["R2"].std(),
            "test_sMAPE_mean": g["sMAPE"].mean(),
            "test_sMAPE_std": g["sMAPE"].std(),
        })
    pd.DataFrame(agg_rows).sort_values("ablation").to_csv(root_out_dir / "tft_sensitivity_summary_by_ablation.csv", index=False)

    manifest = pd.DataFrame([
        {"ablation": "full", "notes": "All baseline TFT schema variables retained."},
        {"ablation": "no_climate", "notes": "Climate unknown reals removed from encoder/decoder unknown inputs."},
        {"ablation": "no_serotype", "notes": "Serotype unknown reals removed from encoder/decoder unknown inputs."},
        {"ablation": "no_temporal", "notes": "TargetMonth_sin and TargetMonth_cos removed from known future covariates."},
        {"ablation": "no_population_density", "notes": "PopulationDensity removed from static real inputs."},
    ])
    manifest.to_csv(root_out_dir / "tft_sensitivity_feature_manifest.csv", index=False)
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="TFT sensitivity analysis for the dengue district-panel pipeline.")
    p.add_argument("--input", type=str, default="data/raw/prime_dataset_model_input_with_purge.csv")
    p.add_argument("--output_dir", type=str, default="TFT/outputs/sensitivity")
    ...
    p.add_argument("--fixed_config_json", type=str, default="")
    ...
    return Config(
        ...
        fixed_config_json=(a.fixed_config_json or None),
...
def main():
    cfg = parse_args()
    if cfg.use_tuning:
        print("Sensitivity mode detected: --use_tuning is ignored. Fixed config/default config is used instead.")
    ...
    ablations = parse_ablation_names(cfg.ablation_names)
    seeds = parse_seed_list(cfg.seed_list)

    experiment_rows: List[Dict[str, Any]] = []
    per_horizon_parts: List[pd.DataFrame] = []

    for ablation in ablations:
        for seed in seeds:
            exp_summary, per_h_df = run_one_experiment(
                df=df,
                base_cfg=cfg,
                windows=windows,
                ablation=ablation,
            ...
            per_horizon_parts.append(per_h_df)

    save_sensitivity_root_tables(root_out_dir, experiment_rows, per_horizon_parts)
