...
def fit_one_horizon(
    ...
    if not horizon_features:
        raise ValueError(f"Horizon {h}: no features available after ablation.")
...
def write_sensitivity_run_summary(cfg, windows, features, out_dir: Path, ablation: str, removed: List[str], include_target_month: bool, seed: int):
    ...
    lines = [
        "Random Forest sensitivity run summary",
        "=" * 80,
        f"Input file: {cfg.input_path}",
        f"Ablation: {ablation}",
        f"Seed: {seed}",
...
def aggregate_sensitivity_outputs(root_dir: Path, experiment_rows: List[Dict], per_h_rows: List[Dict], feature_rows: List[Dict]) -> None:
    ...
    if not exp_df.empty:
        exp_df.to_csv(root_dir / "rf_sensitivity_experiments.csv", index=False)
        summary = (
        ...
        summary.to_csv(root_dir / "rf_sensitivity_summary_by_ablation.csv", index=False)

    if not perh_df.empty:
        perh_df.to_csv(root_dir / "rf_sensitivity_per_horizon_long.csv", index=False)
        perh_summary = (
        ...
        perh_summary.to_csv(root_dir / "rf_sensitivity_per_horizon_summary.csv", index=False)

    if not feat_df.empty:
        feat_df.to_csv(root_dir / "rf_sensitivity_feature_manifest.csv", index=False)
...
def run_experiment(
    ...
    save_json(fixed_out, exp_dir / "rf_sensitivity_used_params.json")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Random Forest sensitivity analysis for the dengue district-panel pipeline.")
    p.add_argument("--input", type=str, default="./data/raw/prime_dataset_model_input_with_purge.csv")
    p.add_argument("--output_dir", type=str, default="RF/outputs/sensitivity")
