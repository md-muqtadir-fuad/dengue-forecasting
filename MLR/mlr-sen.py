...
def apply_ablation(feature_cols: List[str], ablation_name: str) -> Tuple[List[str], List[str]]:
    ablation_name = str(ablation_name).strip().lower()
    base = list(feature_cols)

    if ablation_name == "full":
        return base, []

    if ablation_name == "no_climate":
        removed = [c for c in base if c in CLIMATE_FEATURES]
    elif ablation_name == "no_serotype":
        removed = [c for c in base if c in SEROTYPE_FEATURES]
    elif ablation_name == "no_temporal":
        removed = [c for c in base if c in TEMPORAL_FEATURES]
    elif ablation_name == "no_population_density":
        removed = [c for c in base if c in POPULATION_FEATURES]
    else:
        raise ValueError(
            f"Unknown ablation '{ablation_name}'. "
            "Expected one of: full, no_climate, no_serotype, no_temporal, no_population_density."
        )

    kept = [c for c in base if c not in removed]
    if not kept:
        raise ValueError(f"Ablation '{ablation_name}' removed all features, which is not allowed.")
    return kept, removed
...
def run_one_experiment(
    ...
    exp_dir = root_out_dir / ablation_name
    ensure_dir(exp_dir)

    exp_features, removed_features = apply_ablation(base_features, ablation_name)

    exp_cfg_dict = dict(cfg.__dict__)
    exp_cfg_dict["ablation_name"] = ablation_name
    exp_cfg_dict["base_feature_count"] = len(base_features)
    exp_cfg_dict["feature_count_after_ablation"] = len(exp_features)
    exp_cfg_dict["removed_features"] = removed_features
    save_json(exp_cfg_dict, exp_dir / "run_config.json")

    exp_notes = list(notes)
    exp_notes.append(f"Sensitivity ablation: {ablation_name}")
    exp_notes.append(f"Removed features ({len(removed_features)}): {removed_features if removed_features else 'None'}")

    write_run_summary(cfg, windows, exp_features, exp_dir, audit, exp_notes)

    date_to_split = build_date_split_map(df, cfg)
    results = []
    coverage_frames = []
    for h in range(1, cfg.horizon + 1):
        sample = build_horizon_sample(df, exp_features, cfg, h, date_to_split)
        coverage_frames.append(build_coverage_manifest(sample, windows, h))
        res = fit_one_horizon(sample, exp_features, cfg, h, exp_dir, mase_denom)
        results.append(res)

    pd.concat(coverage_frames, ignore_index=True).to_csv(exp_dir / "mlr_target_coverage.csv", index=False)
    ...
    save_coefficients(results, exp_dir)
    save_diagnostics(results, exp_dir)
    make_figures(results, preds, exp_dir)

    summary = compute_metrics(preds["test"]["y_true_count"], preds["test"]["y_pred_count"], mase_denom)
    summary.update({
    ...
    per_h_rows = []
    for r in results:
        row = {"ablation_name": ablation_name, "horizon": r["horizon"], "feature_count": len(r["feature_cols"])}
    ...
    }, exp_dir / "sensitivity_experiment_summary.json")

    return {
        "ablation_name": ablation_name,
        "exp_dir": str(exp_dir),
        "summary": summary,
        "per_horizon_rows": per_h_rows,
        "removed_features": removed_features,
        "features_used": exp_features,
    }
...
def parse_args() -> Tuple[Config, List[str]]:
    p = argparse.ArgumentParser(description="Sensitivity analysis for direct multi-horizon MLR on the dengue district-panel pipeline.")
    p.add_argument("--input", type=str, default="./data/raw/prime_dataset_model_input_with_purge.csv")
    p.add_argument("--output_dir", type=str, default="MLR/outputs/sensitivity")
    ...
    p.add_argument(
        "--ablation_names",
        type=str,
        default="full,no_climate,no_serotype,no_temporal,no_population_density",
    ...
    ablations = [x.strip() for x in str(a.ablation_names).split(",") if x.strip()]
    return cfg, ablations


def main():
    cfg, ablations = parse_args()
    root_out_dir = Path(cfg.output_dir)
    ensure_dir(root_out_dir)
    save_json(cfg.__dict__ | {"ablation_names": ablations}, root_out_dir / "run_config.json")

    df = load_dataset(cfg)
    audit = audit_panel_structure(df, cfg)
    save_json(audit, root_out_dir / "mlr_data_audit.json")

    date_to_split = build_date_split_map(df, cfg)
    windows = split_windows_from_map(date_to_split)
    df, base_features, notes = get_feature_columns(df, cfg)
    ...
    for ablation_name in ablations:
        ...
        exp_rows.append(res["summary"])
        per_h_rows.extend(res["per_horizon_rows"])
        removed_rows.append({
            "ablation_name": ablation_name,
            "removed_features": ", ".join(res["removed_features"]) if res["removed_features"] else "",
            "features_used": ", ".join(res["features_used"]),
        })

    pd.DataFrame(exp_rows).sort_values("ablation_name").to_csv(
        root_out_dir / "mlr_sensitivity_experiments.csv", index=False
    )
    pd.DataFrame(per_h_rows).sort_values(["ablation_name", "horizon"]).to_csv(
        root_out_dir / "mlr_sensitivity_per_horizon_long.csv", index=False
    )

    if per_h_rows:
        per_h_df = pd.DataFrame(per_h_rows)
        metric_cols = [c for c in ["RMSE", "MAE", "R2", "CVRMSE", "NRMSE", "sMAPE", "MBE", "MedAE", "MASE", "precision", "recall", "f1", "specificity", "roc_auc", "pr_auc"] if c in per_h_df.columns]
        summary_by_ablation = per_h_df.groupby("ablation_name", as_index=False)[metric_cols].mean(numeric_only=True)
        summary_by_ablation.to_csv(root_out_dir / "mlr_sensitivity_summary_by_ablation.csv", index=False)

        summary_by_h = per_h_df.groupby(["ablation_name", "horizon"], as_index=False)[metric_cols].mean(numeric_only=True)
        summary_by_h.to_csv(root_out_dir / "mlr_sensitivity_per_horizon_summary.csv", index=False)

    pd.DataFrame(removed_rows).to_csv(root_out_dir / "mlr_sensitivity_feature_manifest.csv", index=False)
