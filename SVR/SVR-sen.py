...
TEMPORAL_FEATURES = {"Year", "Month_sin", "Month_cos"}
...
def build_ablation_features(base_features: List[str], ablation: str) -> Tuple[List[str], List[str]]:
    ...
    if ablation == "no_climate":
        ...
        removed = [f for f in feats if f in TEMPORAL_FEATURES]
        feats = [f for f in feats if f not in TEMPORAL_FEATURES]
...
def fit_one_horizon(
    ...
    y_train_count = train_df["y_future_count"].values
    threshold = float(fixed_threshold) if fixed_threshold is not None else (float(np.quantile(y_train_count, 0.90)) if len(y_train_count) else 0.0)
    w_train = np.where(y_train_count >= threshold, 3.0, 1.0)

    pipeline = make_pipeline(cfg, fixed_params)
    pipeline.fit(X_train, y_train_log, svr__sample_weight=w_train)
    joblib.dump(pipeline, out_dir / f"svr_model_h{h}.joblib")
    ...
    perm = permutation_importance(
        pipeline,
        X_test,
        test_df["y_future_log"].astype(float).values,
        n_repeats=20,
        random_state=cfg.random_state,
        scoring="neg_mean_absolute_error",
        n_jobs=-1,
    ...
    return {
        ...
        "pipeline": pipeline,
        "feature_cols": features,
        "params": {**cfg.base_params, **(fixed_params or {})},
        "threshold": threshold,
        "pred_train": pred_train,
        "pred_val": pred_val,
        "pred_test": pred_test,
...
def write_sensitivity_run_summary(cfg, windows, features, out_dir: Path, ablation: str, removed: List[str], seed: int):
    lines = [
        "SVR sensitivity run summary",
        "=" * 80,
        f"Input file: {cfg.input_path}",
        f"Ablation: {ablation}",
        f"Seed: {seed}",
...
def summarize_experiment(preds: Dict[str, pd.DataFrame], mase_denom: float, ablation: str, seed: int, experiment: str, feature_count: int) -> Dict:
    test = preds["test"]
...
def aggregate_sensitivity_outputs(root_dir: Path, experiment_rows: List[Dict], per_h_rows: List[Dict], feature_rows: List[Dict]) -> None:
    ...
    if not exp_df.empty:
        exp_df.to_csv(root_dir / "svr_sensitivity_experiments.csv", index=False)
        summary = (
        ...
        summary.to_csv(root_dir / "svr_sensitivity_summary_by_ablation.csv", index=False)

    if not perh_df.empty:
        perh_df.to_csv(root_dir / "svr_sensitivity_per_horizon_long.csv", index=False)
        perh_summary = (
            ...
            .agg(
                ...
                f1_mean=("f1", "mean"),
                SupportVectorRatio_mean=("SupportVectorRatio", "mean"),
            )
            .reset_index()
        )
        perh_summary.to_csv(root_dir / "svr_sensitivity_per_horizon_summary.csv", index=False)

    if not feat_df.empty:
        feat_df.to_csv(root_dir / "svr_sensitivity_feature_manifest.csv", index=False)
...
def run_experiment(
    ...
    ablated_features, removed = build_ablation_features(base_features, ablation)
    write_sensitivity_run_summary(cfg_exp, windows, ablated_features, exp_dir, ablation, removed, seed)
    ...
    save_json(fixed_out, exp_dir / "svr_sensitivity_used_params.json")
    ...
    for r in results:
        ...
        row["SupportVectors"] = r["support_count"]
        row["SupportVectorRatio"] = r["support_ratio"]
    ...
    feature_row = {
        ...
        "features_used": "|".join(list(ablated_features)),
    }
    return exp_summary, per_h_rows, feature_row


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SVR sensitivity analysis for the dengue district-panel pipeline.")
    p.add_argument("--input", type=str, default="./data/raw/prime_dataset_model_input_with_purge.csv")
    p.add_argument("--output_dir", type=str, default="SVR/outputs/sensitivity")
